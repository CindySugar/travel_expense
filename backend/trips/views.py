...
from .services import mark_settlement, money, rebuild_splits, sync_settlements, trip_summary
...
def trips_view(request):
    if request.method == 'GET':
        trips = Trip.objects.filter(Q(owner=request.user) | Q(members__user=request.user)).distinct().select_related('owner')
