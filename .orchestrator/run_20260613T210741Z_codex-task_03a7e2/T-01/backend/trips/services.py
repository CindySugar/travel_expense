...
def trip_card_summary(trip):
    expenses = _prefetched_relation_items(trip, 'expenses')
    members = _prefetched_relation_items(trip, 'members')
    settlements = _prefetched_relation_items(trip, 'settlements')

    total_amount = money(sum((expense.amount for expense in expenses), Decimal('0.00')))
    bill_count = len(expenses)

    timestamps = [trip.updated_at]
    timestamps.extend(expense.created_at for expense in expenses if getattr(expense, 'created_at', None))
    timestamps.extend(expense.updated_at for expense in expenses if getattr(expense, 'updated_at', None))
    timestamps.extend(member.created_at for member in members if getattr(member, 'created_at', None))
    timestamps.extend(settlement.created_at for settlement in settlements if getattr(settlement, 'created_at', None))
    timestamps.extend(settlement.paid_at for settlement in settlements if getattr(settlement, 'paid_at', None))

    last_updated_at = max((ts for ts in timestamps if ts), default=trip.updated_at)
    today = timezone.localdate()
    if trip.start_date and today < trip.start_date:
        status = 'planned'
    elif trip.end_date and today > trip.end_date:
        status = 'completed'
    elif trip.start_date or trip.end_date or bill_count > 0 or len(members) > 1 or settlements:
        status = 'ongoing'
    else:
        status = 'draft'
